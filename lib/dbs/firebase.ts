...
import {ItexpayData, SessionProps, UserData} from '../../types';
...
export async function setItexpay({ user }: SessionProps) {
    if (!user) return null;

    const { email, id, username } = user;
    const ref = doc(db, 'users', String(id));
    const data: ItexpayData = { email };
...
//     const storeHash = contextString?.split('/')[1] || '';
//     const documentId = `${userId}_${storeHash}`; // users can belong to multiple stores
//     const ref = doc(db, 'itexpaySettings', documentId);
//     const itexpaySettings = await getDoc(ref);
//
//     // Set admin (store owner) if installing/ updating the app
//     // https://developer.bigcommerce.com/api-docs/apps/guide/users
//     if (accessToken) {
//         // Create a new admin user if none exists
//         if (!itexpaySettings.exists()) {
//             await setDoc(ref, { storeHash, isAdmin: true });
//         } else if (!itexpaySettings.data()?.isAdmin) {
//             await updateDoc(ref, { isAdmin: true });
//         }
//     } else {
//         // Create a new user if it doesn't exist
