    ...
    return (
        <Panel id="products">
            <Table
                ...
                // ]}
                columns={[
                    { header: 'SN', hash: 'stock', render: ({ stock }) => renderStock(stock), isSortable: true },
                    { header: 'Customer', hash: 'name', render: ({ id, name }) => renderName(id, name), isSortable: true },
                    { header: 'Amount', hash: 'price', render: ({ price }) => renderPrice(price), isSortable: true },
                    { header: 'OrderId', hash: 'stock', render: ({ stock }) => renderStock(stock), isSortable: true },
                    { header: 'TransactionId', hash: 'name', render: ({ id, name }) => renderName(id, name), isSortable: true },
                    { header: 'Date', hash: 'price', render: ({ price }) => renderPrice(price), isSortable: true },
                    // { header: 'Action', hideHeader: true, hash: 'id', render: ({ id }) => renderAction(id) },
                ]}
                items={tableItems}
                itemName="Products"
